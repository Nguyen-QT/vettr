...
- [ ] **6.3: Client Max End Time** (Supersedes the backlog's former "Optional Client Max End Time" idea. Client-facing only, scoped down from the original artist-side-cap-plus-client-preference pairing: an artist who needs to restrict duration on their end already has business hours (4.3) for that. Purely advisory -- an optional per-booking "must be finished by" time the client flags on intake, for the artist to weigh when deciding duration/slot count during review. Never auto-enforced or blocking, matching "No Auto-Booking" -- the artist always has final say), decomposed per the Mandatory Task Breakdown Rule:
  ...
  - [ ] 6.3.2: Domain Service (`clientIntakeInputSchema`/`ClientIntakeInput` gain an optional client end-time field, combined with `requestedDate` the same way `combineRequestedDateAndTime` works; `submitIntakeRequest` persists it; `getPendingIntakeRequests`/`PendingIntakeRequestSummary` expose it for the artist's review. No new Controller/Action layer -- existing actions just gain the field. Unit tests).
