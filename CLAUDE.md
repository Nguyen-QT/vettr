...
- [ ] **7.1: Deposit Payment Gateway Integration** (Integrate Stripe PaymentIntents/Checkout to collect required deposits upon request approval or slot lock-in. Scoped to client deposits only -- artist subscription billing, floated as a maybe in the original idea, is deferred to its own future roadmap item rather than bundled in here. Deposit collection stays decoupled from `RequestStatus`: approval/slot-lock works exactly as it does today, and once a request is `APPROVED`, the client dashboard offers a "Pay Deposit" step if their tier has a configured amount and `depositPaid` is still false -- no new status value, no change to the existing approve/confirm state machine. Artists set their own deposit amount per `ComplexityTier` rather than a platform-wide fixed percentage. Uses embedded Stripe Elements/PaymentIntents so the payment form stays in-app and on-brand, rather than a redirect to Stripe's hosted Checkout), decomposed per the Mandatory Task Breakdown Rule:
  ...
  - [ ] 7.1.3: Domain Service -- Create Payment Intent (`createDepositPaymentIntent(intakeRequestId, clientProfileId)` in `billing`, ownership-checked against the trusted session's `clientProfileId` same as intake's `cancelIntakeRequest`; requires status `APPROVED`, `depositPaid: false`, and a configured tier amount; creates a Stripe PaymentIntent for that amount via a new `src/lib/stripe.ts` client singleton, persists `stripePaymentIntentId`/`depositAmount`; unit tests mocking the Stripe SDK).
  - [ ] 7.1.4: Domain Service -- Confirm Payment (`confirmDepositPayment(paymentIntentId)` in `billing`, idempotent, called from the webhook handler; flips `depositPaid: true` on success; unit tests covering idempotency and mismatched/failed intents).
  - [ ] 7.1.5: Controller/Action (`createDepositPaymentIntentAction`, `setArtistDepositSettingsAction`, and the Stripe webhook route `src/app/api/webhooks/stripe/route.ts` -- signature-verified via `STRIPE_WEBHOOK_SECRET`, delegates to 7.1.4; thin wrappers only).
  - [ ] 7.1.6: UI Primitive & Config (`@stripe/stripe-js` + `@stripe/react-stripe-js` install, publishable-key config, a shared Elements provider wrapper; zero business logic).
  - [ ] 7.1.7: Domain Hook & Logic (`useDepositPayment` -- client-side Elements/card-confirmation state; `useArtistDepositSettings` -- artist settings form state, mirroring `useArtistScheduleSettings`).
  - [ ] 7.1.8: View & Route ("Pay Deposit" card on the client dashboard for an eligible request; a "Deposit Amounts" section on the artist settings page; e2e spec, noting which parts need the user's own manual click-through in Stripe test mode).
