...
// Domain Service (CLAUDE.md 7.1.3): creates the Stripe PaymentIntent a
// client pays to secure an already-APPROVED booking. clientProfileId is
// the trusted session's own id (see intake's cancelIntakeRequest
// precedent) -- an ownership mismatch is reported the same as a missing
// request. depositAmount is snapshotted onto the IntakeRequest here
// rather than read live from ArtistDepositSetting on every future
// check, so a later change to the artist's per-tier amount never
