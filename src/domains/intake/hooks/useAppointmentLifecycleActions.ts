...
// list once resolved.
export function useAppointmentLifecycleActions(intakeRequestId: string) {
  ...
  function cancel() {
    ...
    startTransition(async () => {
      const result = await cancelApprovedBookingAsArtistAction(intakeRequestId);
      if (!result.success) {
  ...
  function markNoShow() {
    setError(null);
    startTransition(async () => {
      const result = await markAppointmentNoShowAction(intakeRequestId);
      if (!result.success) {
        setError(result.error);
        return;
      }
  ...
  function markCompleted() {
    setError(null);
    startTransition(async () => {
      const result = await markAppointmentCompletedAction(intakeRequestId);
      if (!result.success) {
        setError(result.error);
        return;
      }
