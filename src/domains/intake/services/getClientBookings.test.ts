...
describe("getClientBookings", () => {
  ...
  it("does not include another client's booking", async () => {
    ...
    await prisma.intakeRequest.deleteMany({ where: { clientId: otherClientId } });
  ...
  it("orders bookings by createdAt descending", async () => {
    ...
    const older = await prisma.intakeRequest.create({
      data: {
        clientId,
        artistId,
        tier: "TIER_2",
    ...
    const newer = await prisma.intakeRequest.create({
      data: {
        clientId,
        artistId,
        tier: "TIER_2",
