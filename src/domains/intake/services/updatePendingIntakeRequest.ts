...
import type {
  UpdatePendingIntakeRequestInput,
...
// own id (see intake/actions.ts), never client-supplied. Only PENDING
...
// neither is safe to silently move here (see cancelIntakeRequest for
// the cancel path both still support). Re-validates the new requested
// time against live availability, same check the intake form itself
// uses -- the request has no TimeSlot rows of its own yet at this
// status, so there's no self-conflict to worry about.
export async function updatePendingIntakeRequest(
  input: UpdatePendingIntakeRequestInput
