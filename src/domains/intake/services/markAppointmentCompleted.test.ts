...
describe("markAppointmentCompleted", () => {
  ...
  it("marks a past-dated APPROVED appointment as completed", async () => {
    ...
    const result = await markAppointmentCompleted({ intakeRequestId: requestId });

    expect(result).toEqual({ success: true });
    const updated = await prisma.intakeRequest.findUnique({ where: { id: requestId } });
  ...
  it("never applies a cancellation strike", async () => {
    ...
    await markAppointmentCompleted({ intakeRequestId: requestId });
  ...
  it("rejects a request id that does not exist", async () => {
    const result = await markAppointmentCompleted({ intakeRequestId: randomUUID() });
