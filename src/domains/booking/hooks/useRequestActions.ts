...
export function useRequestActions(bookingRequestId: string) {
  ...
  function review(durationMinutes: number, estimatedPrice: number) {
    ...
    startTransition(async () => {
      const result = await reviewBookingRequestAction(bookingRequestId, {
        durationMinutes,
        estimatedPrice,
      });
      if (!result.success) {
  ...
  function confirmBooking() {
    setError(null);
    startTransition(async () => {
      const result = await confirmProposedBookingAction(bookingRequestId);
      if (!result.success) {
        setError(result.error);
        return;
      }
  ...
  function decline() {
    setError(null);
    startTransition(async () => {
      const result = await declineBookingRequest(bookingRequestId);
      if (!result.success) {
        setError(result.error);
        return;
      }
