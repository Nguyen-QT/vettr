...
import { submitBookingRequest } from "@/domains/booking/actions";
import {
...
interface UseVisualBookingFormArgs {
  artistId: string;
...
export function useVisualBookingForm({
  ...
  // clientBookingInputSchema, blocks submission), this only fires once
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    ...
    const result = await submitBookingRequest(artistId, data);
    ...
    setSubmittedRequestId(result.bookingRequestId);
  });
