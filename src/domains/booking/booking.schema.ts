...
// Date BookingRequest.requestedStartTime actually stores. Local-time
// combination only -- no timezone handling yet, same placeholder scope
// as the rest of booking's fields.
...
// see services/reviewBookingRequest.ts for the domain logic that
...
// getPendingBookingRequests/RequestCard, never enforced against actual
...
// (the superRefine below, submitBookingRequest) already treats an empty
...
// see services/updatePendingBookingRequest.ts for the ownership/status
// guard and the availability re-check. Narrower than
// clientBookingInputSchema below: tier/tags/images aren't editable here.
export const updatePendingBookingRequestInputSchema = z
  .object({
...
// bounds with reviewBookingRequestInputSchema above.
...
// see src/domains/booking/services/validateComplexity.ts for the content
// gatekeeper that judges an already-valid request's tags/notes.
export const clientBookingInputSchema = z
  .object({
